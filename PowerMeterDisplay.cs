...
public class PowerMeterDisplay : MonoBehaviour {
    ...
    public BikeComputerScanManager bikeComputerScanManager;
    ...
    private void ReceivedBackgroundScanData(byte[] data)
    {
        ...
        if (b == 11)
        {
            int num = data[10] | data[11] << 8;
            byte b2 = data[13];
            foreach (AntDevice antDevice in scanResult)
            {
                if (antDevice.deviceNumber == num && antDevice.transType == b2)
                {
                    return;
                }
            }
            Debug.Log("Powermeter sensor found " + num);
            AntDevice antDevice2 = new AntDevice();
            antDevice2.deviceType = b;
            antDevice2.deviceNumber = num;
            antDevice2.transType = b2;
            antDevice2.period = 8182;
            antDevice2.radiofreq = 57;
            antDevice2.name = "PM(" + antDevice2.deviceNumber + ")";
            scanResult.Add(antDevice2);
            bikeComputerScanManager.addButton(num.ToString(), b2, b, antDevice2.name);
            if (autoConnectToFirstSensorFound)
            {
                ConnectToDevice(antDevice2);
            }
        }
    }
